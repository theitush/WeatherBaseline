...
class ChartRenderer {
    ...
    getYAxisLabel(metric) {
        const labels = {
            'max_temperature': 'Maximum Temperature (UTCI °C)',
            'min_temperature': 'Minimum Temperature (UTCI °C)',
    ...
    drawLegend() {
        ...
        // Clear existing HTML legend content
        ...
        legendItems.forEach((item, i) => {
            ...
            if (item.type === "rect") {
                legendIcon.className = 'legend-color';
                legendIcon.style.background = item.color;
                legendIcon.style.opacity = item.opacity;
            } else if (item.type === "line") {
                legendIcon.className = 'legend-line';
                legendIcon.style.background = item.color;
                legendIcon.style.opacity = item.opacity;
                ...
                legendIcon.style.width = '8px';
                legendIcon.style.height = '8px';
                legendIcon.style.opacity = item.opacity;
            }
    ...
    drawHistogramBrackets(currentTemp) {
        ...
        if (this.isMobile()) {
            ...
            // Right bracket (higher percentile)
