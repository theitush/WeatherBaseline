...
export interface SnappedCell {
  /** The cell we'll fetch archive data for (grid lat/lon). */
  cell: Cell;
 ...
 * bundled /cells.csv; parsing keeps only lat/lon (the columns the snap needs).
 */
...
function parseCells(text: string): Cell[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (Number.isFinite(lat) && Number.isFinite(lon)) cells.push({ lat, lon });
  }
  return cells;
}
